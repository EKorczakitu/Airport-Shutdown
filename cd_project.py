...
for cid, nodes in large_communities.items():
    ...
    ax.scatter(lons, lats, color=color_lookup[cid], s=15, alpha=0.9,
               transform=ccrs.PlateCarree(), label=f"Comm {cid}" if len(nodes) > 50 else None)

plt.title(f"All NATO Airports - Communities (Z-Score Backbone)")
plt.legend(loc='lower left', markerscale=2, fontsize='small')
plt.tight_layout()
plt.show()

# =====================================================
# 🗺️ 10. Visualization 2: EUROPE ONLY
# =====================================================
...
# 2. Filter nodes: Only show airports if they are in the Europe list
europe_set = set(europe_nato)
europe_nodes = [n for n, d in G_ACTIVE.nodes(data=True) if d['country'] in europe_set]

# Draw Edges (only if both ends are in Europe for cleanliness, or just all edges involving Eur nodes)
# Let's draw edges where *both* source and target are European to keep the map clean
subgraph_europe = G_ACTIVE.subgraph(europe_nodes)

nx.draw_networkx_edges(
    subgraph_europe, pos, ax=ax,
    width=0.1,
    alpha=0.4,
    edge_color='gray',
    arrows=False
...
# Draw Nodes (Only European ones, colored by community)
for cid, nodes in large_communities.items():
    # Filter nodes for this community to only include Europe
    euro_nodes_in_comm = [n for n in nodes if n in pos and G_ACTIVE.nodes[n]['country'] in europe_set]
    
    if euro_nodes_in_comm:
        lons = [pos[n][0] for n in euro_nodes_in_comm]
        lats = [pos[n][1] for n in euro_nodes_in_comm]
        ax.scatter(lons, lats, color=color_lookup[cid], s=20, alpha=0.9,
                   transform=ccrs.PlateCarree())

plt.title(f"NATO Europe Only - Communities & Dominant Airlines")
plt.tight_layout()
plt.show()
...
# =====================================================
# 📊 NMI Analysis: Countries vs. Airlines
# =====================================================

print("NMI")

# 1. Prepare Node Lists
node_list = list(G_ACTIVE.nodes())
labels_louvain = [partition[n] for n in node_list]

# --- METRIC A: NMI for COUNTRIES ---
# "True Label" = The country the airport is in
labels_country = [G_ACTIVE.nodes[n]['country'] for n in node_list]

nmi_country = normalized_mutual_info_score(labels_country, labels_louvain)
print(f"NMI (Communities - Countries): {nmi_country:.4f}")

# --- METRIC B: NMI for DOMINANT AIRLINE ---
# "True Label" = The airline with the most flights departing this airport
# We must calculate this first for every node in the graph
airport_dominant_airline = []

for n in node_list:
    # Get all routes originating from this airport in the NATO dataset
    # (We use the original dataframe 'routes_nato' for accuracy)
    flights_from_node = routes_nato[routes_nato["Source airport ID"] == n]
    
    if not flights_from_node.empty:
        # Find the most frequent airline ID
        top_airline = flights_from_node["Airline"].mode()[0]
        airport_dominant_airline.append(top_airline)
    else:
        # If isolated or no outgoing flights in subset, mark as 'None'
        airport_dominant_airline.append("None")

nmi_airline = normalized_mutual_info_score(airport_dominant_airline, labels_louvain)
print(f"NMI (Communities - Dominant Airline): {nmi_airline:.4f}")

# --- Interpretation ---
print("-" * 40)
if nmi_country > nmi_airline:
    print(">> CONCLUSION: Geography (Borders) is the stronger driver of community structure.")
else:
    print(">> CONCLUSION: Corporate Strategy (Airline Hubs) is the stronger driver of community structure.")
print("="*40)